//variables from various ids on html
  ...
  function updateFiveDayForecastUI(data) {
    ...
    for (let i = 0; i < 40; i += 8) {
      ...
      dayElement.innerHTML = `
          ...
          <li class="list-group-item   fw-bold"></br> ${date}</li>
          <li class="list-group-item   fw-bold">${cloudCover} <br>high ${roundedTempMax}°F<br>humidity ${humidity}%<br>wind ${roundHighWindSpeed}mph</li>`;
  ...
  function addToCityList(cityName) {
    ...
    if (cityList.find("button").filter(function() { return $(this).text() === cityName; }).length === 0) {
      ...
      let removeButton = $('<button>')
          .addClass('btn custom-btn-muted-red btn-sm')
          .html('&times;')
